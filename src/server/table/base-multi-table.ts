...
import {BaseMultiEntity} from "../../common/entity/base-multi-entity";
...
export class BaseMultiTable<T1 extends BaseMultiEntity, T2 extends MultiTableOptions> extends BaseTable {
  ...
  private parseFindQuery(query: any): any {
    _.merge(query, this.Class.APPEND_QUERY);
  }

  saveLocal(entities: T1|T1[], query: any = null): Promise<void> {
    ...
    return MyPromise.eachFunctionSeries<T1>(arrEntities, (resolve, reject, entity) => {
      ...
      let q = query ? this.parseFindQuery(query) : {_id: entity._id};
      this.datastore.update(q, entity, {upsert: true}, (err, numReplaced) => {
  ...
  removeLocal(query: Object): Promise<void> {
