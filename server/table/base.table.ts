...
export interface ISequelizeInstance<T extends BaseEntity>
  extends sequelize.Instance<T> {}

export interface ISequelizeModel<T extends BaseEntity>
  extends sequelize.Model<T> {}

export interface IBaseTableParams<T extends BaseEntity> {
  fields: sequelize.ModelAttributes;
  options: sequelize.ModelOptions<ISequelizeModel<T>>;
  jsonFields: string[];
}

@injectable()
export abstract class BaseTable<T extends BaseEntity> {
  params: IBaseTableParams<T> = <any>{};

  EntityClass: typeof BaseEntity;

  protected name: string;
  protected archiveName: string = "";
  protected archiveFields: sequelize.ModelAttributes;
  protected archiveOptions: sequelize.ModelOptions<ISequelizeModel<T>>;
  protected sequelizeDatabase: sequelize.Sequelize | null = null;
  protected sequelizeModel: ISequelizeModel<T> | null = null;
  protected archiveSequelizeModel: ISequelizeModel<T> | null = null;
  ...
  initialize(database: sequelize.Sequelize) {
    ...
    this.sequelizeModel = this.sequelizeDatabase.define(this.name, this.Class.params.fields, this.Class.params.options);
    if (this.EntityClass.params.archive) {
      let addIndexes: sequelize.DefineIndexesOptions[] = [];
      _.each(
        this.Class.params.fields,
        (field: sequelize.DefineAttributeColumnOptions, name) => {
      ...
      this.archiveOptions.indexes = _.union(
        addIndexes,
        this.archiveOptions.indexes
      );
      this.archiveSequelizeModel = this.sequelizeDatabase.define(this.archiveName, this.archiveFields, this.archiveOptions);
    }
  }
  ...
  async findOne(options: IFindEntityOptions<T> = {}): Promise<T> {
    ...
    let instance: ISequelizeInstance<T> = await this.sequelizeModel.findOne(
      options
    );
  ...
  async findAll(options: IFindEntityOptions<T> = null): Promise<T[]> {
    options = this.parseFindOptions(options);
    let model = options.archive
      ? this.archiveSequelizeModel
      : this.sequelizeModel;
    this.message("find", ["local"], this.EntityClass.params.name, true, {
      options: options,
    });
  ...
  private parseFindOptions(
    options: IFindEntityOptions<T>
  ): IFindEntityOptions<T> {
    options = options || {};
    options.where =
      options.where || _.cloneDeep(this.EntityClass.params.default.where);
    options.where = _.merge(
      options.where || {},
      this.EntityClass.params.append.where || {}
    );
    options.order =
      options.order || _.cloneDeep(this.EntityClass.params.default.order);
    options.order = _.concat(
      <any>(options.order || []),
      <any>(this.EntityClass.params.append.order || [])
    );
    return options;
  }
  ...
  async save(entity: T, archive: boolean = false): Promise<T> {
    ...
    savedInstance = await this.sequelizeModel
      .build(this.prepareSaveEntity(entity), { isNewRecord: !oldInstance })
      .save();
    ...
    if (this.EntityClass.params.archive && archive) {
      this.message(
        ...
        this.EntityClass.params.name,
        true,
        { primaryKey: entity.primaryKey, displayField: entity.displayField }
      );
      await this.archiveSequelizeModel.create(
        this.prepareSaveEntity(savedEntity)
      );
      this.message(
        "save",
        ["local", "archive"],
  ...
  async saveAll(entities: T[]): Promise<T[]> {
    ...
    this.message("saveAll", ["local"], this.EntityClass.params.name, true, {
      count: entities.length,
    });
    let saveEntities: T[] = [];
    for (let entity of entities) saveEntities.push(await this.save(entity));
    this.message("saveAll", ["local"], this.EntityClass.params.name, false, {
      count: entities.length,
    });
    return saveEntities;
  }
  ...
  protected message(
    ...
    name: string,
    isStart: boolean,
    dispData: Object = null
  ) {
    let message = `${_.startCase(action)} ${_.join(options, " ")} ${name} was ${
