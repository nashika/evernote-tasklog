...
export class SyncService extends BaseServerService {
  ...
  private nextLockPromise: Promise<void> = new Promise(() => {});
  private lockResolves: Array<() => void>;
  private timer: any;
  private interval: number = this.Class.startInterval;
  ...
  async sync(manual: boolean): Promise<void> {
    ...
    try {
      ...
      while (
        ...
        localSyncState.updateCount < remoteSyncState.updateCount
      ) {
  ...
  private async getSyncChunk(
    ...
    if (!localSyncState.updateCount)
      throw new Error("localSyncState.updateCount undefined.");
    ...
    await this.tableService.notebookTable.removeByGuid(
    ...
    await this.tableService.savedSearchTable.removeByGuid(
    ...
    await this.tableService.linkedNotebookTable.removeByGuid(
