...
export default class SettingModel extends Model {
    ...
    loadLocal(key:string, callback:(err?:Error, result?:any) => void):void {
        core.loggers.system.debug(`Load local ${(<typeof SettingModel>this.constructor).PLURAL_NAME} was started. key=${key}`);
        ...
        this._datastore.find(query).sort({}).limit(limit).exec((err, docs) => {
            core.loggers.system.debug(`Load local ${(<typeof SettingModel>this.constructor).PLURAL_NAME} was ${err ? 'failed' : 'succeed'}. docs.length=${docs.length}`);
    ...
    saveLocal(key:string, value:Object, callback:(err?:Error, results?:any) => void):void {
        ...
        this._datastore.update({_id: key}, doc, {upsert: true}, (err, numReplaced, newDoc) => {
            ...
            core.loggers.system.debug(`Upsert ${(<typeof SettingModel>this.constructor).PLURAL_NAME} end. numReplaced=${numReplaced}`);
            callback();
        });
    }
