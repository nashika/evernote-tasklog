...
var TimeLogModel = (function (_super) {
    __extends(TimeLogModel, _super);
    function TimeLogModel() {
        _super.apply(this, arguments);
    }
    TimeLogModel.prototype.parse = function (note, lines, callback) {
        var _this = this;
        var timeLogs = [];
        for (var line in lines) {
            ...
            if (matches = line.match(/(.*)[@＠](\d{2,4}[\/\-]\d{1,2}[\/\-]\d{1,2}.+)/)) {
                ...
                // parse person
                for (var person in core_1["default"].users[this._username].settings.persons) {
    ...
    TimeLogModel.PLURAL_NAME = 'timeLogs';
    TimeLogModel.TITLE_FIELD = 'comment';
    TimeLogModel.DEFAULT_LIMIT = 2000;
    return TimeLogModel;
