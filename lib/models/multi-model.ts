...
export default class MultiModel extends Model {
    ...
    countLocal(options:Object, callback:(err:Error, results?:any) => void):void {
        options = this.__parseFindOptions(options);
        core.loggers.system.debug(`Count local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was started. query=${JSON.stringify(options['query'])}`);
        this._datastore.count(options['query'], (err:Error, count:number) => {
            core.loggers.system.debug(`Count local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was ${err ? 'failed' : 'succeed'}. count=${count}`);
            callback(err, count);
        });
    }
    ...
    private __parseFindOptions(options:Object):Object {
        ...
        result['query'] = options['query'] || merge(true, (<typeof MultiModel>this.constructor).DEFAULT_QUERY);
        ...
        merge(result['query'], (<typeof MultiModel>this.constructor).APPEND_QUERY);
        merge(result['sort'], (<typeof MultiModel>this.constructor).APPEND_SORT);
        return result;
    }
    ...
    saveLocal(docs:Array<Object> | Object, callback:(err?:Error, results?:any) => void):void {
        ...
        async.eachSeries(arrDocs, (doc, callback) => {
            ...
            core.loggers.system.debug(`Save local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was ${err ? 'failed' : 'succeed'}. docs.count=${docs['length']}`);
            callback(err);
        });
    }
    ...
    saveLocalUpdateOnly(docs:Array<Object> | Object, callback:(err?:Error, results?:any) => void):void {
        ...
        if (!docs || docs['length'] == 0) return callback();
        if (!Array.isArray(docs)) arrDocs = [docs];
        core.loggers.system.debug(`Save local update only ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was started. docs.count=${docs['length']}`);
        async.eachSeries(arrDocs, (doc, callback) => {
            var localDoc = null;
            async.waterfall([
                (callback) => {
                ...
                (docs, callback) => {
                    ...
                    if (localDoc && localDoc.updateSequenceNum >= doc['updateSequenceNum']) {
                        core.loggers.system.trace(`Upsert local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was skipped. guid=${doc['guid']}, title=${doc[(<typeof MultiModel>this.constructor).TITLE_FIELD]}`);
                        callback();
                    } else {
                        core.loggers.system.trace(`Upsert local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was started. guid=${doc['guid']}, title=${doc[(<typeof MultiModel>this.constructor).TITLE_FIELD]}`);
                        async.waterfall([
            ...
            core.loggers.system.debug(`Save local update only ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was ${err ? 'failed' : 'succeed'}. docs.count=${docs['length']}`);
            callback(err);
        });
    }
    ...
    removeLocal(query:Array<string> | string | Object, callback:(err?:Error, results?:any) => void):void {
        ...
        core.loggers.system.debug(`Remove local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was started. query=${JSON.stringify(query)}`);
        this._datastore.remove(query, {multi: true}, (err, numRemoved) => {
            core.loggers.system.debug(`Remove local ${(<typeof MultiModel>this.constructor).PLURAL_NAME} was ${err ? 'failed' : 'succeed'}. numRemoved=${numRemoved}`);
            callback(err);
        });
    }
