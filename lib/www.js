...
var MySyncChunk = (function (_super) {
    __extends(MySyncChunk, _super);
    function MySyncChunk() {
        _super.apply(this, arguments);
    }
    return MySyncChunk;
})(evernote.Evernote.SyncChunk);
var Www = (function () {
    function Www() {
    ...
    Www.prototype.main = function (app, server) {
        ...
        core_1["default"].models.settings = new setting_table_1.SettingTable();
        ...
        async.waterfall([
            ...
            function (callback) {
                core_1["default"].users[username].models = {
                    linkedNotebooks: new linked_notebook_table_1.LinkedNotebookTable(username),
                    notes: new note_table_1.NoteTable(username),
                    notebooks: new notebook_table_1.NotebookTable(username),
                    profitLogs: new profit_log_table_1.ProfitLogTable(username),
                    searches: new search_table_1.SearchTable(username),
                    settings: new setting_table_1.SettingTable(username),
                    syncStates: new sync_state_table_1.SyncStateTable(username),
                    tags: new tag_table_1.TagTable(username),
                    timeLogs: new time_log_table_1.TimeLogTable(username),
                    users: new user_table_1.UserTable(username)
                };
                callback();
            },
            // Initialize datas
